<?php
	...
	require_once('../static/sec/mysql_info_guest.php');
	$line_array = ["\r\n", "\r", "\n"];
?>
...
<?php
	...
	if($prev_month==1){
		...
		$prev_2_month=$prev_month-1;
	}
	$table_name='shift_month'.$month.'_guest';
	$prev_table_name='shift_month'.$prev_month.'_guest';
	$prev_2_table_name='shift_month'.$prev_2_month.'_guest';
	$days=date('d',strtotime('last day of '.$year_month));

	try{
		$pdo=new PDO('mysql:host=localhost;dbname='.$mysql_info_guest['dbname'].';charset=utf8',$mysql_info_guest['user'],$mysql_info_guest['pass']);
		if(!($pdo->query('SELECT * FROM '.$table_name))){
			$pdo->query('RENAME TABLE '.$prev_table_name.' to '.$table_name.')');
			$pdo->query('RENAME TABLE '.$prev_2_table_name.' to '.$prev_table_name.')');
		}
	}
	...
	<head>
		...
		<script>
			$(function(){
				...
				//確定シフト、提出シフト表示判別変数
				<?php
				$fr=fopen('text_guest/confirmed.txt','r');
				$confirmed=fgets($fr);
				$confirmed=str_replace($line_array,'',$confirmed);
				fclose($fr);
				$fr=fopen('text_guest/hide.txt','r');
				$hide=fgets($fr);
				...
				$('select[name="day"]').on('change',function(){
					...
					let p_id=$('select[name="full_time_employee"]').val()
					for(let i=2;i<=<?php echo $employee_count; ?>;i++){
						let p_day=$('select[name="day"]').val();
						let str=$('#'+i+'_'+p_day).text();
						let part_name=$('#'+i).text();
				...
				<div class="scroll">
					<table>
						...
						try{
							...
							#店長
							if($_SESSION['id']==1){
								$sql=$pdo->query('SELECT * FROM user_data_guest INNER JOIN '.$table_name.' ON user_data_guest.id='.$table_name.'.id'); ?>
							<?php }
							#その他
							else{
								$sql=$pdo->query('SELECT * FROM user_data_guest INNER JOIN '.$table_name.' ON user_data_guest.id='.$table_name.'.id'); ?>
				...
				<div class="scroll">
					<table>
						<tr>
							...
							try{
								$pdo=new PDO('mysql:host=localhost;dbname='.$mysql_info_guest['dbname'].';charset=utf8',$mysql_info_guest['user'],$mysql_info_guest['pass']);
								$sql=$pdo->query('SELECT * FROM user_data_guest INNER JOIN '.$prev_table_name.' ON user_data_guest.id='.$prev_table_name.'.id');
						...
						try{
							$pdo=new PDO('mysql:host=localhost;dbname='.$mysql_info_guest['dbname'].';charset=utf8',$mysql_info_guest['user'],$mysql_info_guest['pass']);
							$sql=$pdo->query('SELECT * FROM user_data_guest INNER JOIN '.$prev_table_name.' ON user_data_guest.id='.$prev_table_name.'.id ORDER BY user_data_guest.id'); ?>
						<?php foreach($sql as $row){ ?>
							<?php array_push($employee_names,$row['name']); ?>
							<?php array_push($employee_ids,$row['id']); ?>
							<?php if($row['id']==$_SESSION['id']) { ?>
